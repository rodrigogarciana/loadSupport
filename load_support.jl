...
function writeUNC(linkingTable,nameV,n2i,i2n)
    ...
    rRowLength=findlast([linkingTable[i][11] for i in 1:length(linkingTable)].=="r");
    hRowLength=findlast([linkingTable[i][11] for i in 1:length(linkingTable)].=="h")-rRowLength;
    cRowLength=findlast([linkingTable[i][11] for i in 1:length(linkingTable)].=="c")-(rRowLength+hRowLength);
    ###
...
function remove_noise(a,eps)
    ...
    elseif typeof(a)==Dict{Any,Any}
        for i in keys(a) for j in keys(a[i])
...
function i2nD(i2nDict,prop)
    outputDict=Dict();
    for i in eachindex(prop)
        outputDict[i2nDict[i]]=prop[i]
    end
    return outputDict
end


function n2iA(n2iDict,prop)
    outputArray=Array{Union{Missing,Int64,Float64}}(missing,length(n2iDict));
    for i in eachindex(n2iDict)
        outputArray[n2iDict[i]]=prop[i]
    end
    return outputArray
end
...
function generateTablesfromODS(path::String,filename::String)
    basepath=pwd();
    cd(path*"/");
   ...
   # folders=dropComments(folders)
    ...
    folders=ods_read(filename;sheetName="main",retType="DataFrame")
    folders=dropComments(folders)

    #indices=[DataFrame(XLSX.readtable(folders.Filename[i],"index")) for i in eachindex(folders.Filename)]
    indices=[ods_read(folders.Filename[i];sheetName="index",retType="DataFrame") for i in eachindex(folders.Filename)]
    indices=dropComments.(indices)
    
    #tables=[[dropComments(DataFrame(XLSX.readtable(folders.Filename[i],iS(indices[i].sheet_name[j])))) for j in eachindex(indices[i].sheet_name)] for i in eachindex(folders.Filename)]; ### This is the slow one
    tables=[[dropComments(ods_read(folders.Filename[i];sheetName=iS(indices[i].sheet_name[j]),retType="DataFrame")) for j in eachindex(indices[i].sheet_name)] for i in eachindex(folders.Filename)]; ### This is the slow one
...
function fastTransformStructure(mpDict::Dict,ntype::String)

    structureDefinition(mpDict,ntype);
    mp_type=eval(Meta.parse(ntype*"_type"))
    ...
    for n in eachindex(fields)
        if types[n]==Float64 || types[n]==Int64 || types[n]==String
            if types[n]==Float64 stpar*="0.0" elseif types[n]==Int64  stpar*="0" elseif types[n]==String stpar*="\"\"" end
        elseif types[n]==UnitRange{Int64} stpar*="0:-1"
        else
            stpar*="$(types[n])()"
        end
    ...
    for i in eachindex(fields)
        if types[i]!=UnitRange{Int64}
            types[i]!=Float64 ? setfield!(mp,fields[i],convert(types[i],mpDict[string(fields[i])])) : setfield!(mp,fields[i],mpDict[string(fields[i])])
        else
            setfield!(mp,fields[i],mpDict[string(fields[i])][1]:mpDict[string(fields[i])][end])
            @assert getfield(mp,fields[i])==mpDict[string(fields[i])]
        end
    end
...
function structureText(obj,nameV)
...
function drop_miss!(obj)
    fln=fieldnames(typeof(obj))
    for fn in fln
        if typeof(getfield(obj,fn))<:Dict 
            objFld=getfield(obj,fn)
            try 
                missingVals=ismissing.(collect(values(objFld))) 
                missingKeys=collect(keys(objFld))[missingVals]
                for k in missingKeys delete!(objFld,k) end
            catch
            end

            if typeof(objFld)<:Dict
                lok=collect(keys(objFld))
                for key in lok
                    try 
                        missingVals=ismissing.(collect(values(objFld[key]))) 
                        missingKeys=collect(keys(objFld[key]))[missingVals]
                        for k in missingKeys delete!(objFld[key],k) end
                    catch
                    end
                end
            end
...
function partition_list(list, num_partitions)
    chunk_size = div(length(list), num_partitions)
    partitions = []
    ...
    # Last partition may have a different size
    push!(partitions, list[(num_partitions-1)*chunk_size+1:end])
...
@printf("done in %4.1fs\n",round(time()-a,digits=1))
